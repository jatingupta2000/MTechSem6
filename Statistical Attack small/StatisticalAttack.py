...
def execute_verilog(arg, key):
    arg = arg[1:]
    f = open('./Execution/small_input.txt', 'w')
    input_str = ''.join(str(x) for x in arg)
    key_str = ''.join(str(x) for x in key)
    f.write(input_str+key_str)
    f.close()
    os.system('iverilog -o ./Execution/small.vvp ./Execution/small.v ./Execution/small_testBench.v')
    os.system('vvp ./Execution/small.vvp')
    f = open('./Execution/small_output.txt', 'r')
    lines = f.read().splitlines()
    f.close()
    return [int(x) for x in [*lines[-1]]]

def execute_oracle(arg, time_period):
    os.system('./Execution/small '+' '.join(str(x) for x in arg) + ' ' + str(time_period))
    f = open('./b15_2ip_OP.txt', 'r')
    lines = f.read().splitlines()
    f.close()
    return [int(x) for x in [*lines[-1]]]
